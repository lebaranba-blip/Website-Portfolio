...
Запускай дев-сервер на `http://localhost:3000` **до любой работы** — он нужен для скриншотов и визуальной проверки.
...
| Аспект | Деталь |
|--------|--------|
| **Проблема** | При скролле контент пр1
...
## 📦 ПОСЛЕДНИЕ ИЗМЕНЕНИЯ (сессия 2026-04-19)

### Завершён кейс Gump Donuts — `/works/gump`

**Статус:** ✅ DONE — страница полностью реализована и задеплоена

**Новые файлы в `public/works/gump/`:**
- `pipeline.png` — схема пайплайна (Разбор воркфлоу пайплайна.png)
- `pipeline.mp4` — видео воркфлоу (видео пайплайн.mp4)
- `workflow-board.png` — скриншот Kling-борды (Воркфлоу.PNG) — используется в секции AI Pipeline вместо pipeline.png
- `space1.png` — фотореалистичный рендер: астронавт за рулём розового фургона в космосе
- `space-video.mp4` — видео с брендированным фургоном в космосе

**Структура страницы `/works/gump`:**
```
Header (← Все работы / Написать)
Hero: poster.png + заголовок (float-loop анимация на постере)
— 01 Идея: текст концепции + logo/billboard 2-col + "EAT OR REGRET." bleed
— 02 Identity: hex-палитра (Gump Blue/Donut Pink/Night/Glaze) + masonry-галерея с lightbox
— 03 AI Pipeline: тёмная секция #0D0D0D, workflow-board.png, pipeline.mp4, space1.png + space-video.mp4, 3 тезиса
— 04 В движении: VideoPlayer ad.mp4 с кнопкой звука
— 05 О проекте: 3 карточки Задача/Инструменты/Результат
CTA: Telegram + ← Все работы
```

**Технические детали:**
- Lightbox: отдельные наборы для poster (POSTER_ITEM) и галереи (IDENTITY_ITEMS)
- Hero-постер: `maxHeight: 70vh` для мобайла + `animate={{ y: [0,-8,0] }}` float-loop
- Hex-коды под цветовыми свотчами
- Текст переписан через humanizer
...
## AI Avatar `/works/ai-avatar` — ✅ DONE (сессия 2026-04-13)
